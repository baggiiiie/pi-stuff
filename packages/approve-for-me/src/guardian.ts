...
function formatApprovalPrompt(
    ...
    const assessment = fresh
        ? [
            `Goal alignment: ${formatPercent(fresh.goalAlignedProbability)}`,
            `Context sufficiency: ${formatPercent(fresh.contextSufficientProbability)}`,
        ]
        : [
            "TypeSafe could not evaluate this command, so human attention is required.",
            `Reason: ${safeErrorMessage(error)}`,
        ];

    return [
        ...assessment,
        ...(timeout === undefined ? [] : [`Timeout: ${timeout} seconds`]),
        "",
        "Command:",
